...
class PhotoReviewCommandServiceTest {
    ...
    void create_정상저장_리뷰번호반환() {
        ...
        when(repository.save(any(PhotoReviewEntity.class))).thenAnswer(inv -> {
            PhotoReviewEntity e = inv.getArgument(0);
            e.setReviewNo(1000);
            return e;
        });
    ...
    void edit_부분업데이트_널아닌필드만반영() {
        ...
        when(repository.save(any(PhotoReviewEntity.class))).thenAnswer(inv -> inv.getArgument(0));

        UpdateRequest req = mock(UpdateRequest.class);
        when(req.getBoardNo()).thenReturn(null);                    // 유지
        when(req.getMemberNo()).thenReturn(222);                    // 변경
        when(req.getReviewTitle()).thenReturn("new title");         // 변경
        when(req.getReviewDate()).thenReturn(null);                 // 유지
        when(req.getReviewContent()).thenReturn("new content");     // 변경
        when(req.getReviewLike()).thenReturn(5);                    // 변경
    ...
    void delete_대상있으면_삭제후_1반환() {
        ...
        verify(repository).deleteById(321);
    }
}
